...
if ($checkUserPermissions['permissions'] == 'all' || in_array('procedure_add', $checkUserPermissions['permissions'])) {
?>
	<script>
	$(function(){
		//ADD
		...
		$(document).on('submit', '#add-service-modal form', function(event) {
			...
			$.post('<?=BASEURL."procedure/add"?>', {data: $form.serialize()}, function(resp) {
				resp = $.parseJSON(resp);
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title" id="myLargeModalLabel">Add Procedure</h4>
...
<?php
if ($checkUserPermissions['permissions'] == 'all' || in_array('procedure_edit', $checkUserPermissions['permissions'])) {
?>
	<script>
	$(function(){
		...
		$(document).on('submit', '#edit-service-modal form', function(event) {
			...
			$.post('<?=BASEURL."procedure/update"?>', {data: $form.serialize()}, function(resp) {
				resp = $.parseJSON(resp);
