...
if ($checkUserPermissions['permissions'] == 'all' || in_array('lab_test_category_add', $checkUserPermissions['permissions'])) {
?>
	<script>
	$(function(){
		//ADD
		...
		$(document).on('submit', '#add-service-modal form', function(event) {
			...
			$.post('<?=BASEURL."laboratory/add-cat"?>', {data: $form.serialize()}, function(resp) {
				resp = $.parseJSON(resp);
...
<?php
if ($checkUserPermissions['permissions'] == 'all' || in_array('lab_test_category_edit', $checkUserPermissions['permissions'])) {
?>
	<script>
	$(function(){
		...
		$(document).on('submit', '#edit-service-modal form', function(event) {
			...
			$.post('<?=BASEURL."laboratory/update-cat"?>', {data: $form.serialize()}, function(resp) {
				resp = $.parseJSON(resp);
	...
	<div class="modal fade bd-example-modal-lg" id="edit-service-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title" id="myLargeModalLabel">Edit Lab Test Category</h4>
					<button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
				</div><!-- /modal-header -->
				<div class="modal-body">
